#!/bin/bash
# Setup AI Skills for PrimeFire API development
...
show_help() {
    ...
    echo ""
    echo "Configure AI coding assistants for PrimeFire API development."
    echo ""
    echo "Options:"
...
setup_copilot() {
    if [ -f "$REPO_ROOT/AGENDA.md" ]; then
        mkdir -p "$REPO_ROOT/.github"
        cp "$REPO_ROOT/AGENDA.md" "$REPO_ROOT/.github/copilot-instructions.md"
...
copy_agents_md() {
    ...
    agents_files=$(find "$REPO_ROOT" -maxdepth 1 -name "AGENDA.md" -not -path "*/node_modules/*" -not -path "*/.git/*" 2>/dev/null)
...
echo " PrimeFire API AI Skills Setup"
echo "================================"
echo ""
...
echo -e "${BLUE}      AGENDA.md is the source of truth - edit it, then re-run this script.${NC}"
