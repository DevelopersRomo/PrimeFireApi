...
def to_snake_case(name):
    ...
    for i, char in enumerate(name):
        if char.isupper():
            if i > 0:
                if name[i-1].islower() or (i > 1 and name[i-2].islower()):
                    result.append('_')
...
def convert_sql_file(input_path, output_path=None):
    ...
    fk_patterns = [
        ...
        (r'FK_ticketAttachments_TicketMessageId_ticketMessages', 'fk_ticket_attachments_ticket_message_id_ticket_messages'),
        ...
        (r'FK_TimeSheetLocationSnapshots_CustomerId_Customers', 'fk_time_sheet_location_snapshots_customer_id_customers'),
        (r'FK_TimeSheetLocationSnapshots_EmployeeId_Employees', 'fk_time_sheet_location_snapshots_employee_id_employees'),
