...
def generate_partial_backup(tables_with_data=None, target_table=None, db_prefix="DB", backup_dir=None) -> None:
    """
    ...
    output_file = os.path.join(  # noqa: PTH118
        backup_dir,
        f"partial_backup_{database_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql",  # noqa: DTZ005
    )
    ...
    with pathlib.Path(output_file).open("w", encoding="utf-8") as f:
        ...
        f.write(
            f"/****** This script contains ALL table structures and data for: {', '.join(tables_with_data)} ******/\n\n"
        )
